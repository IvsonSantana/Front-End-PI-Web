...
const API_ALUNOS_URL = 'http://localhost:3000/api/users/alunos'; // Endpoint para buscar alunos
...
const API_TURMAS_URL = 'http://localhost:3000/api/turmas';
...
function abrirModalAlunosDaTurma(turmaId) {
    const token = localStorage.getItem('token');
    fetch(`http://localhost:3000/api/turmas/${turmaId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json' ,
            'Authorization': `Bearer ${token}` // Adiciona o token no cabeçalho
...
async function deletarAlunoDaTurma(turmaId, alunosIds) {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:3000/api/turmas/delete/${turmaId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json' ,
                'Authorization': `Bearer ${token}` // Adiciona o token no cabeçalho
